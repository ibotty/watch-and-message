...
main = do
    ...
    runLog (loglevel config) . flip evalStateT (kafkaConfig config) $ do
        ...
        logDebug $ LogEntry "Starting" [("config", format w config)]
...
inotifyHandler
  :: (MonadBaseControl IO io, MonadThrow io, MonadState KafkaState io, MonadLog (WithSeverity LogEntry) io, MonadIO io)
  => MoveConfig -> FilePath -> io ()
inotifyHandler config file = when (Path.filename file == "sha256sum.txt") $ do
